...
# --- locate V3_ROOT (experimental copy lives at <V3>/script/experimental/) ---
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
V3_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
cd "${V3_ROOT}"
echo "[run_all-EXPERIMENTAL] V3_ROOT=${V3_ROOT}"

# --- tunables ---
HOLDOUT="${HOLDOUT:-B1,Tregs,plasma}"
...
echo "[run_all] seed_rule=${SEED_RULE}"
...
echo "============================================================"
python script/experimental/train_main_model.py \
    --run_tag _cache_build_only_exp \
    --rebuild_cache \
    --max_epochs 0 \
...
echo "============================================================"
python script/summarize_results.py --include_tags "exp_"
...
echo "[run_all] per-run reports: ${V3_ROOT}/runs/exp_*"
