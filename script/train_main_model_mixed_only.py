...
# -*- coding: utf-8 -*-
"""
V3 — Cell-type aware MTI prediction (DIAGNOSTIC: train on MIXED PAIRS only).

This is a copy of train_main_model.py with a single added behaviour:

    --train_mixed_only  (default: ON in this script)
        After the cache is loaded, the training split is filtered down to rows
        whose (mirna_gid, utr_gid) pair has BOTH y=0 and y=1 examples across
        cells (i.e. mixed pairs only — pure pairs are discarded).

The reason: pure pairs are solvable by a UTR-only model (label is determined
by UTR identity alone). Mixed pairs are the ONLY samples that *require* the
model to use cell context. Training on mixed pairs only therefore forces the
model into cell-conditioned reasoning instead of falling into the UTR-identity
shortcut. This is a *diagnostic* experiment, not a production setting:
"is this pair mixed?" requires knowing labels across cells, which is a label
leak at inference time. Report it as such in Results §4.x.

The cache, evaluation splits, model architecture, ablation variants, loss and
weighting code are 100% unchanged. Only the train rows fed to the optimiser
are filtered, and the per-row sample_weight + UTR-only early-stop baseline
are recomputed on the filtered train.

Run after the regular cache is built (e.g. after run_all.sh has finished
step 1, or after any regular training run with the same holdout config):

    python script/train_main_model_mixed_only.py \\
        --run_tag diag_mixed_film_seed42 \\
        --model film \\
        --batch_size 256 --num_workers 8 --device auto \\
        --max_epochs 10 --patience 3 \\
        --holdout_cell_types "B1,Tregs,plasma,mDC1" \\
        --val_holdout_cell_types "mDC1" \\
        --seed_rule any_7mer

(or use the helper:  ./script/run_mixed_only.sh)
...
# ---------------------------------------------------------------------------
# Mixed-pair-only filter  (diagnostic mode for this script)
# ---------------------------------------------------------------------------
...
def filter_train_to_mixed_only(cache: dict, logger: Logger) -> dict | None:
    ...
    After filtering we ALSO recompute:
      - sample_weight  using the same (1/sqrt(group_size)) * (1 + 3*purity)
        formula as write_cache, normalised to mean = 1 on the filtered set
      - UTR-only PR-AUC baselines for val / val_utr_holdout /
        val_cell_holdout / val_utr_x_cell_holdout  using train_utr_avg derived
        from the FILTERED train (since that is what the model actually sees)

    Returns the dict of recomputed baselines, or None if train was empty.
    """
    ...
    # ---- apply mask in place to every aligned array in the train dict ----
    ...
    # ---- recompute sample_weight on filtered train ----
    ...
    # ---- recompute UTR-only PR-AUC baselines using filtered train ----
    train_pos_sum: dict[int, float] = {}
    train_count: dict[int, int] = {}
...
def collate_batch(batch):
    ...
    weight = torch.from_numpy(np.asarray([x[5] for x in batch], dtype=np.float32))
    y = torch.from_numpy(np.asarray([x[6] for x in batch], dtype=np.float32))
    return mirna_x, utr_x, utr_vlen, cell_x, ts_feats, weight, y
...
def make_model(n_cell_types: int, model_kind: str = "full",
    ...
    """model_kind:
      ...
      'film'     : miRNA seq + UTR attention-pooled, but cell embedding is injected as
                   Feature-wise Linear Modulation (Perez et al. 2018) on the UTR feature
                   map (gamma, beta = MLP(cell_emb); u_seq = (1+gamma)*u_seq + beta) instead
                   of being concatenated at the head. Tests whether the WAY we inject cell
                   information matters (vs. simply concat in 'full').

    use_ts_features: if True, concatenate the 5-d TargetScan hand-crafted features
    (z-scored at cache build time) at the head BEFORE the final classifier. These
    are precomputed per (mirna_gid, utr_gid) pair and act as a TargetScan-style
    inductive prior — especially important for held-out-UTR generalization where
    the conv branches cannot rely on memorizing UTR sequences.
    """
    import torch
    ...
    USE_ATTN = model_kind in {"full", "no_cell", "seq_only", "film"}  # mean-pool when False
    USE_FILM = (model_kind == "film")
    CELL_CONCAT_AT_HEAD = USE_CELL and not USE_FILM  # film mode injects cell ONLY via FiLM
    ...
    class CellAwareMTI(nn.Module):
        def __init__(self):
            ...
            if USE_FILM:
                # produce per-channel gamma, beta for the 128-channel UTR feature map
            ...
            if USE_TS:
                # Small MLP on the 5-d hand-crafted features. Concat at head.
                ts_hidden = 16
        ...
        def forward(self, mirna_x, utr_x, utr_vlen, cell_x, ts_feats=None):
            ...
            if self.use_seq:
                ...
                # === Optional FiLM: modulate UTR feature map by cell embedding ===
                if self.use_film and cell_emb is not None:
                    ...
                    # broadcast across positions: u_seq <- (1+gamma)*u_seq + beta
...
def evaluate(model, loader, device):
    ...
    with torch.no_grad():
        for mirna_x, utr_x, utr_vlen, cell_x, ts_feats, _weight, y in loader:
            ...
            logits = model(mirna_x, utr_x, utr_vlen, cell_x, ts_feats)
...
def train_one_epoch(model, loader, loss_fn, opt, device, logger, log_every: int, epoch: int):
    ...
    t0 = time.time()
    for bidx, (mirna_x, utr_x, utr_vlen, cell_x, ts_feats, weight, y) in enumerate(loader, start=1):
        mirna_x = mirna_x.to(device, non_blocking=True)
        utr_x = utr_x.to(device, non_blocking=True)
        utr_vlen = utr_vlen.to(device, non_blocking=True)
        cell_x = cell_x.to(device, non_blocking=True)
...
class Config:
    ...
    train_mixed_only: bool = True       # DIAGNOSTIC: filter train rows to mixed pairs only
...
def parse_args() -> Config:
    ...
    p.add_argument("--no_train_mixed_only", action="store_true",
                   help="DISABLE the mixed-pair filter on train rows (default in this script: ON). "
                        "When ON, only pairs whose label varies across cells are kept in the "
                        "training set; pure pairs are dropped. This is a diagnostic-only mode.")
    a = p.parse_args()
    holdout_list = None
    ...
    return Config(
        ...
        train_mixed_only=(not a.no_train_mixed_only),
    )
...
def main():
    ...
    # ---- Phase 2.5: DIAGNOSTIC -- mixed-pair-only train filter ----
    # In this diagnostic script the default is ON. Pure pairs (label invariant
    # across cells) are dropped from training so the model is forced to learn
    # cell-conditioned reasoning rather than UTR-identity memorisation.
    # All validation / test splits remain untouched, so reported numbers on
    # test_utr_holdout, test_cell_holdout etc. are directly comparable to the
    # full-train baseline produced by the regular train_main_model.py.
    ...
    if cfg.train_mixed_only:
        ...
        logger.log("[note] --no_train_mixed_only set; running with full train (= main script behaviour)")
    ...
    # In mixed-only mode we override these with baselines recomputed from the
    # filtered train (since that is what the model actually sees), so the
    # "lift" composite reflects improvement over what THIS training set's
    # UTR-only trivial predictor would achieve.
    ...
    if mixed_only_baselines is not None:
        ...
        # also persist to run_dir for later inspection
    ...
    model = make_model(n_cell_types, cfg.model_kind, cfg.cell_embed_dim,
                       ...
                       ts_feature_dim=TARGETSCAN_FEATURE_DIM).to(device)
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    logger.log(f"[model] kind={cfg.model_kind} trainable_params={n_params:,}")
